public class MySubArray 
{
    public static void subarraySum(int[] arr) 
    {
        // printing all subArrays

        for (int i = 0; i < arr.length; i++) 
        {
            for (int j = i; j < arr.length; j++) 
            {
                // System.out.println("Index of i : "+i);
                // System.out.println("Index of j : "+j);

                // System.out.println("___________________________________");

                // // Print elements from i to j
                System.out.print("{ ");
                for (int k = i; k <= j; k++) 
                {
                    System.out.print(arr[k] + " "); 
                }
                System.out.println("}");
            }
        }
    }

    public static void main(String[] args) 
    {
        int arr[] = {5,3,7};

        subarraySum(arr);
    }
}

/*
int arr[] = {5,3,7};

-------------------------------
    index :     0 -  1 -  2   
-------------------------------
    element :   5 -  3 -  7
-------------------------------

---------------------------------------------------------------------------------------------------------------------------------------------

    This are the indexes               Elements as below  
    (for code use Loop As above code)   (for code use inside loop : "arr[loop wali indexes]")  
    (indexes for to go till location )  ( actual elements for any logic )

_____________________________________________________________________________________________________________________________________________

    Index of i : 0                  { 5 }   | { 1 }
        to
    Index of j : 0

    for (int k = 0; k <= 0; k++)
_____________________________________________________________________________________________________________________________________________

    Index of i : 0
        to                          { 5 3 }  | { 1 2 }
    Index of j : 1

    for (int k = 0; k <= 1; k++)
_____________________________________________________________________________________________________________________________________________

    Index of i : 0
        to                          { 5 3 7 } | { 1 2 3 }
    Index of j : 2

    for (int k = 0; k <= 2; k++)
_____________________________________________________________________________________________________________________________________________

    Index of i : 1
        to                          { 3 }    |  { 2 }
    Index of j : 1

    for (int k = 1; k <= 1; k++)
_____________________________________________________________________________________________________________________________________________

    Index of i : 1
        to                          { 3 7 }   | { 2 3 }
    Index of j : 2

    for (int k = 1; k <= 2; k++)
_____________________________________________________________________________________________________________________________________________

    Index of i : 2
        to                            { 7 }   |  { 3 }
    Index of j : 2

    for (int k = 2; k <= 2; k++)
_____________________________________________________________________________________________________________________________________________

#MORAL OF THE STORY ( MARATHI )
    NOTE : (this is positive numbers)
        ( Brute-force approach )
    1.  jrr mla direct array sobat khelaycha asel (simple array [ one by one element ])
    like (addition trr) nested looping madhe kam hoil.

    2.  jrr mla SUB-ARRAY sobat khelaycha aahe trr 3 nested-loop madhe 
    jave lagnar like above code.

 */